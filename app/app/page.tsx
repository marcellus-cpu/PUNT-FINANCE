...
function GlobalHeader() {
  return (
    <header style={{ borderBottom: "1px solid var(--color-divider)" }} className="sticky top-0 z-50">
      <div style={{ backgroundColor: "rgba(250,249,246,0.96)", backdropFilter: "blur(8px)" }}>
        <div style={{ borderBottom: "1px solid var(--color-divider)", backgroundColor: "var(--color-oxford)" }} className="w-full py-1.5 px-4 sm:px-8 flex items-center justify-between">
          <p className="label-fin" style={{ color: "var(--color-champagne)", letterSpacing: "0.18em" }}>
            Democratising High Finance · Kueleza Fedha za Kimataifa
          </p>
          <div className="flex items-center gap-2">
            <Globe size={10} style={{ color: "var(--color-champagne)", opacity: 0.7 }} />
            <button
        ...
        <nav role="navigation" aria-label="Primary navigation" className="max-w-screen-xl mx-auto px-4 sm:px-8 py-4 flex items-center justify-between">
          <a href="/" aria-label="Punt Finance — Home" className="flex items-baseline gap-2">
            <span style={{ fontFamily: "var(--font-serif)", color: "var(--color-brass)", fontSize: "1.375rem", fontWeight: 700 }} aria-hidden="true">₱</span>
...
function HeroSection() {
  return (
    <section
      ...
      style={{ borderBottom: "1px solid var(--color-divider)", backgroundColor: "var(--color-cream)" }}
      className="w-full"
    >
      <div className="max-w-screen-xl mx-auto px-4 sm:px-8 py-20 sm:py-28 lg:py-32 flex flex-col items-center text-center">
        <div className="flex items-center gap-3 mb-8">
          ...
          <p className="label-fin" style={{ color: "var(--color-brass)", letterSpacing: "0.2em" }}>The Simplification Engine</p>
          <span className="ornament-rule" aria-hidden="true" />
        </div>
        <h1
          id="hero-heading"
    ...
    <article className="book-card" aria-label={`${book.title} by ${book.author}`}>
      ...
      <div style={{ padding: "1.5rem" }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span style={{ fontFamily: "var(--font-sans)", fontSize: "0.625rem", color: "var(--color-muted)", letterSpacing: "0.06em" }}>{book.levelEn}</span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <p style={{ fontFamily: "var(--font-sans)", fontSize: "0.5625rem", color: "var(--color-muted)", letterSpacing: "0.08em", textTransform: "uppercase" }}>ISBN: {book.isbn}</p>
...
function LibrarySection({ books }: { books: BookItem[] }) {
  return (
    <section id="library" aria-labelledby="library-heading" style={{ backgroundColor: "var(--color-parchment)", borderBottom: "1px solid var(--color-divider)" }} className="w-full">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-8 py-20 sm:py-24">
        <div className="flex items-end justify-between mb-10" style={{ borderBottom: "1px solid var(--color-divider-dark)", paddingBottom: "1.25rem" }}>
          <div>
            <p className="label-fin mb-2" style={{ color: "var(--color-muted)" }}>§02 — Financial Literacy</p>
          ...
          <div style={{ fontFamily: "var(--font-sans)", fontSize: "0.6875rem", color: "var(--color-muted)", letterSpacing: "0.06em", textAlign: "right", lineHeight: 1.6 }}>
            <p>Mwanzo · Kati · Mtaalam</p>
            <p>Beginner · Intermediate · Professional</p>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px" role="list" style={{ backgroundColor: "var(--color-divider)" }}>
          {books.map((book) => (
            <div key={book.id} role="listitem" style={{ backgroundColor: "var(--color-parchment)" }}>
              <BookCard book={book} />
            </div>
          ))}
        </div>
...
function SiteFooter() {
  ...
  return (
    <footer role="contentinfo" style={{ backgroundColor: "var(--color-oxford)", borderTop: "1px solid var(--color-oxford-dim)" }} className="w-full">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-8 py-12">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 mb-10">
          <div>
            <div className="flex items-baseline gap-2 mb-3">
              <span style={{ fontFamily: "var(--font-serif)", color: "var(--color-brass)", fontSize: "1.25rem", fontWeight: 700 }} aria-hidden="true">₱</span>
              <span style={{ fontFamily: "var(--font-serif)", color: "var(--color-cream)", fontSize: "1rem", fontWeight: 500, letterSpacing: "0.05em" }}>Punt Finance</span>
            </div>
          ...
          <nav aria-label="Footer navigation">
            <p className="label-fin mb-4" style={{ color: "var(--color-brass)" }}>Navigation</p>
          ...
          <div>
            <p className="label-fin mb-4" style={{ color: "var(--color-brass)" }}>Legal</p>
            <ul className="list-none flex flex-col gap-2.5">
...
export default function HomePage() {
  return (
    <>
      <a href="#main-content" style={{ position: "absolute", top: "-100%", left: "1rem", backgroundColor: "var(--color-oxford)", color: "var(--color-cream)", padding: "0.5rem 1rem", fontFamily: "var(--font-sans)", fontSize: "0.875rem", zIndex: 9999 }} className="focus:top-2">
